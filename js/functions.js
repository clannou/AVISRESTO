...
function restoInfoWindow(values){
    ...
    values.forEach( (resto) =>{
      ...
      let streetViewImagesrc = "https://maps.googleapis.com/maps/api/streetview?location=" + resto.lat + "," + resto.long + "&size=300x200&key=AIzaSyC9hPFyow-eEse7HOUvLgSAv_GhxZ5l1og"
...
function addResto(values){
    ...
    const addRestoContentString =
      `<div id="addRestoContent">
    ...
    map.addListener("click", (mapsMouseEvent) => {
      ...
      map.setCenter(addRestoinfoWindow.position); // je centre la map sur cet endroit
        ...
        let newRestoFirstComment = document.getElementById("first-comment").value;
...
function showVisibleMarkers(values) {
    ...
    values.forEach((resto) => {
        ...
        if(bounds.contains(resto.position)===true && resto.marker.setMap(map) != false) { // ET SI MARKER.SHOW= TRUE
...
function starsFilterSlider(values) {
    ...
    $( "#slider-range" ).on( "slidechange", function( event, ui ) {
      ...
      values.forEach((resto) => {
        ...
        if (resto.ratingsAverage >= starMin & resto.ratingsAverage <= starMax) {
          // count++
          resto.marker.setMap(map);
          $(resto.restodivlisting).show()
          // $('#resto-count h4 span').html(count);
        } else if (resto.ratingsAverage <= starMin || resto.ratingsAverage >= starMax) {
            // count--
...
function addPlaces(values){
    ...
    function createMarker(place) {
      ...
      if (userview.contains(place.geometry.location) === true) {
